...
SWEP.VElements = {
	["redbird"] = { type = "Model", model = "models/svrkdstuff/terence.mdl", bone = "ValveBiped.Grenade_body", rel = "", pos = Vector(2.105, 0.866, -1.068), angle = Angle(-133.818, 39.349, -6.362), size = Vector(0.196, 0.196, 0.196), color = Color(255, 255, 255, 255), surpresslightning = false, material = "", skin = 0, bodygroup = {} }
}
...
function SWEP:PrimaryAttack()
    ...
    if SERVER then
        ...
        timer.Simple(0, function()
            ...
            self:EmitSound("svrkdstuff/terence_fly.mp3")
            ...
            timer.Simple(0.2, function()
                ...
                angrybird:SetModel("models/svrkdstuff/terence.mdl")
                angrybird:SetAngles(ply:EyeAngles())
                angrybird:Spawn()
				
				self.ActiveTerence = angrybird

                local phys = angrybird:GetPhysicsObject()
                if IsValid(phys) then
                    phys:SetVelocity(ply:GetAimVector() * 6500)
                end
...
function SWEP:SecondaryAttack()
    if SERVER then
        local bird = self.ActiveTerence
        if not IsValid(bird) then return end
        self:SetNextSecondaryFire(CurTime() + 0.5)
        self:EmitSound("svrkdstuff/terence_squeak.mp3")
        self.ActiveTerence = nil
    end
end
