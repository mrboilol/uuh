...
net.Receive("hostage_start",function()
...
local teams = {
	[0] = {
		objective = "Prevent the hostage from being rescued.",
		name = "a Terrorist",
...
hook.Add( "StartCommand", "TDM_DisallowMoveOrShoting", function( ply, mv )
	...
	if zb.CROUND == "hostage" and (zb.ROUND_START or 0) + 20 > CurTime() then 
...
function MODE:HUDPaint()
    ...
    draw.SimpleText("ZBattle | "..(self.PrintName or "Hostage Rescue"), "ZB_HomicideMediumLarge", sw * 0.5, sh * 0.1, Color(0,162,255, 255 * fade), TEXT_ALIGN_CENTER, TEXT_ALIGN_CENTER)
